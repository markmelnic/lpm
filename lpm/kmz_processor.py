...
class KMZ:
    ...
    def _arrange_df(self) -> None:
        for i, row in self.df.iterrows():
            sub_df = self.df.loc[
                (self.df["north"] == row["north"]) & (self.df["south"] == row["south"])
            ]
            if not sub_df.empty:
    ...
    def _generate_image(
        ...
        if horizontal:
            ...
            vertical_set = [
                self._generate_image(image, horizontal=True) for image in images
            ]
    ...
    def coords_item(self, coords: list) -> list:
        ...
        for item in self.globe_matrix[gset[0] : gset[1]]:
            for i, row in item.iloc[sset[0] : sset[1]].iterrows():
                if (row["north"] >= coords[0] >= row["south"]) and (
                    row["west"] <= coords[1] <= row["east"]
                ):
                    return row.tolist()

    def load_images(
        self, images: list, single: bool = False, neighbours: bool = False
    ) -> list:
        if single:
            if neighbours:
                f = [images[:-7], images[-4:]]
                ...
                return edges, self._generate_image(
                    self.load_images(images), fullvh=True
                )
            else:
            ...
            if images:
                if type(images[0]) == list:
                    kmz_imgs = [
                        [
                            Image.open(
                                self.kmz_zip.open(ZIP_KMZ_IMG_FOLDER + "/" + image)
                            )
                    ...
                    kmz_imgs = [
                        Image.open(self.kmz_zip.open(ZIP_KMZ_IMG_FOLDER + "/" + image))
                        for image in images
                    ]
            else:
                kmz_imgs = [
                    Image.open(self.kmz_zip.open(image))
                    for image in self.kmz_zip.namelist()
                    if image.split("/")[0] == ZIP_KMZ_IMG_FOLDER
                ]
            return kmz_imgs
    ...
    def save_images(self, images: list = []) -> None:
        ...
        if not images:
            images = [
                self.load_images(matrix["image"].tolist())
